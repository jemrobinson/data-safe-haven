...
def main():
    ...
    with suppress(IndexError):
        _ = list(
            filter(
                lambda x: x["event_type"] == "start"
                and x["name"] == "azure-ds/write_files",
                cloud_init_log_events,
            )
        )[0]
        end_entries = list(
            filter(
                lambda x: x["event_type"] == "finish"
                and x["name"] == "azure-ds/write_files",
                cloud_init_log_events,
            )
        )
    ...
    with suppress(IndexError):
        end_entries = list(
            filter(
                lambda x: x["event_type"] == "finish" and x["name"] == "modules-config",
                cloud_init_log_events,
            )
        )
        if end_entries:
    ...
    with suppress(IndexError):
        end_entries = list(
            filter(
                lambda x: x["event_type"] == "finish"
                and x["name"] == "modules-final/config-package-update-upgrade-install",
                cloud_init_log_events,
            )
        )
        if end_entries:
    ...
    for event in events:
        ...
        log(
            event["timestamp"],
            event["level"],
            f"{event['message']}: {human_readable(time_elapsed)}",
        )
    ...
    with suppress(ZeroDivisionError):
        ...
        log(
            timestamp,
            "INFO",
            f"..... mean usage: {mem_mean:>6.2f}% => {(mem_gb * mem_mean / 100):>4.1f} GB",
        )
        log(
            timestamp,
            "INFO",
            f"...... min usage: {mem_min:>6.2f}% => {mem_gb * mem_min / 100:>4.1f} GB",
        )
        log(
            timestamp,
            "INFO",
            f"...... max usage: {mem_max:>6.2f}% => {mem_gb * mem_max / 100:>4.1f} GB",
        )
        # CPU
        ...
        log(
            timestamp,
            "INFO",
            f"..... mean usage: {cpu_mean:>6.2f}% => {(n_cores * cpu_mean / 100):>4.1f} cores",
        )
        log(
            timestamp,
            "INFO",
            f"...... min usage: {cpu_min:>6.2f}% => {(n_cores * cpu_min / 100):>4.1f} cores",
        )
        log(
            timestamp,
            "INFO",
            f"...... max usage: {cpu_max:>6.2f}% => {(n_cores * cpu_max / 100):>4.1f} cores",
        )
        ...
        for fname in glob.glob("/opt/monitoring/python-*-safety-check.json"):
            ...
            if packages:
                ...
                log(
                    timestamp,
                    "WARNING",
                    f"Safety check found problems with Python {python_version}",
                )
            for package in packages:
                log(
                    timestamp,
                    "WARNING",
                    f"... {package[0]} [{package[2]}] is affected by issue {package[4]} (for versions {package[1]})",
                )
