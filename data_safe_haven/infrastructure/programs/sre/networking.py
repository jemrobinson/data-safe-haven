    ...
    def __init__(
        ...
        nsg_data_configuration = network.NetworkSecurityGroup(
            ...
            security_rules=[
                ...
                network.SecurityRuleArgs(
                    access=network.SecurityRuleAccess.ALLOW,
                    description="Allow inbound connections from traffic filter containers.",
                    destination_address_prefix=subnet_data_configuration_prefix,
                    destination_port_range="*",
                    direction=network.SecurityRuleDirection.INBOUND,
                    name="AllowTrafficFilterInbound",
                    priority=NetworkingPriorities.INTERNAL_SRE_TRAFFIC_FILTER,
                    protocol=network.SecurityRuleProtocol.ASTERISK,
                    source_address_prefix=subnet_traffic_filter_prefix,
                    source_port_range="*",
                ),
                network.SecurityRuleArgs(
                    access=network.SecurityRuleAccess.ALLOW,
        ...
        nsg_traffic_filter = network.NetworkSecurityGroup(
            ...
            security_rules=[
                ...
                network.SecurityRuleArgs(
                    access=network.SecurityRuleAccess.ALLOW,
                    description="Allow outbound connections over the internet.",
                    destination_address_prefix="Internet",
                    destination_port_ranges=[Ports.HTTP, Ports.HTTPS],
                    direction=network.SecurityRuleDirection.OUTBOUND,
